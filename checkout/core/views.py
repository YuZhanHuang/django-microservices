...
class LinkAPIView(APIView):
    ...
    def get(self, _, code=''):
        ...
        user = UserService.get(f'users/{link.user_id}')
        data['user'] = user
...
class OrderAPIView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            source_str = "source={CHECKOUT_SESSION_ID}"
            source = stripe.checkout.Session.create(
                success_url=f'{os.getenv("CHECKOUT_URL")}/success?{source_str}',
                cancel_url=f'{os.getenv("CHECKOUT_URL")}/error',
...
class OrderConfirmAPIView(APIView):
    def post(self, request):
        ...
        data['ambassador_email'] = str(order.ambassador_email)
